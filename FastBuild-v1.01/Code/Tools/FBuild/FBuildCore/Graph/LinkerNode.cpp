...
/*virtual*/ Node::BuildResult LinkerNode::DoBuild( Job * job )
{
    ...
    for (;;)
    {
        ...
        if ( !spawnOK )
        {
            ...
            FLOG_ERROR( "Failed to spawn process '%s' for %s creation for '%s' cmd args:'(%s %s) @dir:%s @env:%s'", 
                    m_Linker.Get(), GetDLLOrExe(), GetName().Get(), m_Linker.Get(), fullArgs.GetFinalArgs().Get(), workingDir, environment);
            return NODE_RESULT_FAILED;
        }
