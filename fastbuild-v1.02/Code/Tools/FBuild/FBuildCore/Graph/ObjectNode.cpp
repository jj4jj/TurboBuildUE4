...
/*static*/ void ObjectNode::HandleSystemFailures( Job * job, int result, const AString & stdOut, const AString & stdErr )
{
    ...
    // General process failures
        ...
        {
            job->Error( "Remote failure: STATUS_INVALID_IMAGE_FORMAT (0xC000007B) - Check Compiler() settings! Tips:\n\t(%s\n\t%s\n\t%s\n", job->GetRemoteName().Get(), stdOut.Get(), stdErr.Get());
            return;
        }
        if ( (uint32_t)result == 0xC0000135 ) // STATUS_DLL_NOT_FOUND
        {
            job->Error( "Remote failure: STATUS_DLL_NOT_FOUND (0xC0000135) - Check Compiler() settings! Tips:\n\t(%s\n\t%s\n\t%s\n", job->GetRemoteName().Get(), stdOut.Get(), stdErr.Get());
            return;
        }
    #endif
