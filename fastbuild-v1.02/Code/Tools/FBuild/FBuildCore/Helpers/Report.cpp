...
void Report::GenerateAndSaveGsReport(const FBuildStats& stats){
    AString OutputContent;
    OutputContent.SetReserved(1024*1024*4);

    OutputContent += "#obj,name,<object count>,<include num>\n";
    OutputContent += "#inc,<included num>,<in pch (1|0)>,<file path>\n";
    ...
    for (LibraryStats** it = m_LibraryStats.Begin(); it != end; ++it) {
        ...
        //obj,name,object num,include num
        //inc,objects num, included num, in pch(1/0),Name
        char buff[20];
        const size_t numIncludes = incStats.GetSize();
        OutputContent += "obj,";
        OutputContent += library->GetName().Get();
        OutputContent += ",";

        snprintf(buff,16, "%u", (*it)->objectCount);
        OutputContent += buff;
        OutputContent += ",";
        snprintf(buff,16, "%zd", numIncludes);
        OutputContent += buff;
        OutputContent += "\n";
        numLibsOutput++;
        ...
        for (size_t i = 0; i < numIncludes; ++i) {
            ...
            OutputContent += "inc,";
            snprintf(buff,16, "%u", included);
            OutputContent += buff;
            OutputContent += ",";
            OutputContent += inPCH?"1":"0";
            OutputContent += ",";
    ...
    FileStream fs;
    if (fs.Open("gsreport.txt", FileStream::WRITE_ONLY)) {
        fs.Write(OutputContent.Get(), OutputContent.GetLength());
    }
