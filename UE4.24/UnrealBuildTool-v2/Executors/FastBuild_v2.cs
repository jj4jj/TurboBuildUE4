...
namespace UnrealBuildTool
{
	...
	class FastBuild_v2 : ActionExecutor
	{
		List<TargetDescriptor> TargetDescriptors;
		ActionExecutor FallbackExcutor;
		public readonly static string DefaultExecutableBasePath = Path.Combine(UnrealBuildTool.EngineDirectory.FullName, "Extras", "FASTBuild");
		public FastBuild_v2(ActionExecutor Fallback, List<TargetDescriptor> TargetDescs)
		{
			FallbackExcutor = Fallback;
			TargetDescriptors = TargetDescs;
		}
		//////////////////////////////////////////
		// Tweakables
		...
		[XmlConfigFile(Category = "BuildConfiguration")]
		public static bool bFastBuildDebug = false;
		...
		private FileReference GetProjectFile()
		{
			for (int i = 0; i < TargetDescriptors.Count; ++i)
			{
				if (TargetDescriptors[i].ProjectFile != null)
				{
					return TargetDescriptors[i].ProjectFile;
				}
			}
			return UnrealBuildTool.GetInstalledProjectFile();
		}
		public override bool ExecuteActions(List<Action> Actions, bool bLogDetailedActionStats)
		{
			...
			if (Actions.Count <= FastbuildActionAtleastNum)
			{
				return FallbackExcutor.ExecuteActions(Actions, bLogDetailedActionStats);
			}
			...
			if (CompileActions.Any())
			{
				...
				FileReference ProjectFile = GetProjectFile();
				string FASTBuildFilePath = Path.Combine(UnrealBuildTool.EngineDirectory.FullName, "Intermediate", "Build", "fastbuild.bff");
				if (ProjectFile != null)
				{
					FASTBuildFilePath = DirectoryReference.Combine(ProjectFile.Directory, "Intermediate", "Build", "fastbuild.bff").FullName;
				}

				for(int FileSeq = 0; ; ++FileSeq)
				{
					var TryTestFile = FASTBuildFilePath.Replace(".bff", $".{FileSeq.ToString("D4")}.bff");
					if (!File.Exists(TryTestFile))
					{
						FASTBuildFilePath = TryTestFile;
						break;
					}
				}


				//use project dir not engine
				...
				//remove the bff file
				System.IO.File.Delete(FASTBuildFilePath);
			}
			...
			if (PostCompileActions.Any())
			{
				...
				bool bResult = FallbackExcutor.ExecuteActions(PostCompileActions, bLogDetailedActionStats);
		...
		private void WriteEnvironmentSetup()
		{
			...
			if (VCEnv != null)
			{
				...
				AddText($"\t.Executable = '$CLFilterRoot$\\fb-cl-filter.exe'\n");//fastbuild-cl-filter 4.24
				AddText($"\t.ExtraFiles =\n\t{{\n");
				AddText($"\t\t'$Root$/cl.exe'\n");
				AddText($"\t\t'$Root$/c1.dll'\n");
				...
				List<String> PotentialMSVCRedistPaths = new List<String>(Directory.EnumerateFiles(string.Format("{0}/Redist/MSVC",
						VCEnv.GetVCInstallDirectory()), "vccorlib"+ platformVersionNumber + ".dll", SearchOption.AllDirectories));

				var PrefferedMSVCRedistPath = PotentialMSVCRedistPaths.FindAll((str) => str.Contains(".CRT") &&
											str.Contains(VCEnv.Architecture.ToString()) && str.Contains(MsvcCRTRedistVersion));

				if (PrefferedMSVCRedistPath.Count() == 0)
				{
					...
					throw new Exception("Not found the vc tool redis msvc lib for your env !");
				}
				var FinalMSVCRedistPath = Path.GetDirectoryName(PrefferedMSVCRedistPath.Last());
		...
		private void AddCompileAction(Action Action, IEnumerable<Action> DependencyActions)
		{
			...
			//remove some options for build happily
			OtherCompilerOptions = OtherCompilerOptions.Replace("/we4668", "");
			OtherCompilerOptions = OtherCompilerOptions.Replace("/wd4668", "");
		...
		private bool ExecuteBffFile(string BffFilePath)
		{
			...
			string VerboseArguments = bFastBuildVerbose ? "-verbose -showcmds -showcmdoutput" : "";
			string DebugArguments = bFastBuildDebug ? "-debug -j1" : "";
			...
			string FBCommandLine = $"-monitor -summary {DistArgument} {CacheArgument} {IDEArgument} -clean -config \"{BffFilePath}\" {NoStopOnErrorArgument} {ForceRemoteArgument} {VerboseArguments} {DebugArguments}";
