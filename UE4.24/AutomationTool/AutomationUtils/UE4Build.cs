...
namespace AutomationTool
{
	...
	public class UE4Build
	{
		...
		public bool CanUseFastBuild(UnrealTargetPlatform Platform)
		{
			//windwos -> windows; mac
			if (BuildHostPlatform.Current.Platform.IsInGroup(UnrealPlatformGroup.Microsoft))
			{
				return (Platform.IsInGroup(UnrealPlatformGroup.Microsoft)) ;
			}
			if (BuildHostPlatform.Current.Platform.IsInGroup(UnrealPlatformGroup.Apple))
			{
				return (Platform.IsInGroup(UnrealPlatformGroup.Apple));
			}
			return false;
		}
		...
		public void Build(BuildAgenda Agenda, bool? InDeleteBuildProducts = null, bool InUpdateVersionFiles = true, bool InForceNoXGE = false, bool InUseParallelExecutor = false, bool InShowProgress = false, int? InChangelistNumberOverride = null, Dictionary<BuildTarget, BuildManifest> InTargetToManifest = null)
		{
			...
			bool bDisableFastBuild = ParseParam("NoFastBuild") || InForceNoXGE;
			...
			bool bCanUseFastBuild = !bDisableFastBuild;
			CommandUtils.LogLog("************************* UE4Build:");
			CommandUtils.LogLog("************************* UseXGE: {0} UseFastBuild: {1}", bCanUseXGE,  bCanUseFastBuild);
			...
			foreach (BuildTarget Target in Agenda.Targets)
			{
				...
				else if (bCanUseFastBuild && CanUseFastBuild(Target.Platform))
				{
					ParallelXgeTargets.Add(Target);
				}
