// Copyright 2020 Kyle. Provided under a MIT License, see license file on github.
// Tested with Windows 10, Visual Studio 2015/2017/2019, Unreal Engine 4.24.3, FastBuild v01.01
// Durango is fully supported (Compiles with VS2019).
...
namespace UnrealBuildTool
{
    class FASTBuild : ActionExecutor
    {
        ...
        public static string FBuildExePathOverride = "FBuild.exe";
        // Controls network build distribution
        private bool bEnableDistribution = true;
        //remote link
        private bool bDisableRemoteLink = false;
        //build clean
        private bool bEnableClean = false;
        ...
        //compiler
        private WindowsCompiler Compiler = WindowsCompiler.VisualStudio2019;
        ...
        private string CachePath = @"\\UE4bBuild\FASTBuildCache";
        ...
        public static bool IsAvailable()
        {
			...
			//engine path
			string tryPath = Path.Combine(UnrealBuildTool.EngineDirectory.MakeRelativeTo(DirectoryReference.GetCurrentDirectory()), "Extras","FASTBuild", "FBuild.exe");
			if (File.Exists(tryPath)) {
				FBuildExePathOverride = tryPath;
				return true;
			}
        ...
        private FileReference ProjectFile = null; //todo
        private void DetectBuildEnvironment(List<Action> Actions)
        {
        ...
        public override bool ExecuteActions(List<Action> Actions, bool bLogDetailedActionStats)
        {
            ...
            if (Actions.Count > 0)
            {
                ...
                DetectBuildEnvironment(Actions);
                ...
                if (CreateBffFile(Actions, FASTBuildFilePath, LocalExecutorActions))
                {
					...
					//Log.TraceWarning("fast build result:{0} report path:{1}/Source/report.html", FASTBuildResult, UnrealBuildTool.EngineDirectory.FullName);
        ...
        private VCEnvironment VCEnv = null;
        private void CheckVCBuildEnvironment()
        {
            if (ProjectFile == null)
            {
                //ProjectFile = new FileReference(UnrealBuildTool.RootDirectory.FullName + "/../unreal_proj/u3.uproject");
            }
            ...
            try
            {
                ...
                if (BuildType == FBBuildType.Windows)
                {
                    ...
                    VCEnv = VCEnvironment.Create(Compiler, UnrealTargetPlatform.Win64, WindowsArchitecture.x64, null, null);
                }
            ...
            {
                Console.WriteLine("Failed to get Visual Studio environment Error:"+e.ToString());
            }
        }
        private void WriteEnvironmentSetup()
        {
            DirectoryReference VCInstallDir = null;
            string VCToolCompilerPath = "";
            ...
            { //vc env
                ...
                if (!WindowsPlatform.TryGetVSInstallDir(Compiler, out VCInstallDir))
                {
                ...
                VCToolCompilerPath = VCEnv.CompilerPath.ToString();//VCEnvironment.GetVCToolPath64(WindowsPlatform.GetDefaultCompiler(null), VCEnv.ToolChainDir).ToString();
                string VCToolDirPath = VCEnv.CompilerPath.Directory.ToString();

                Console.WriteLine("SetFastBuildEnv VCToolCompilerPath=" + VCToolCompilerPath + " PlatformVer=" + platformVersionNumber + " VCInstallDir=" + VCInstallDir + " VS="+ VCEnv.Compiler.ToString());
                ...
                {
                    var numericDirectories = Directory.GetDirectories(VCToolDirPath).Where(d => Path.GetFileName(d).All(char.IsDigit));
                ...
                {
                    AddText(string.Format("\t\t'$Root$/tbbmalloc.dll'\n")); //vs2019 mspdbcore dep
        ...
        private void AddCompileAction(Action Action, int ActionIndex, List<int> DependencyIndices)
        {
            ...
            Console.WriteLine("compile object index:"+ActionIndex+" compile options:" + string.Join(",", ParsedCompilerOptions.Keys) + " is mvc:" + IsMSVC().ToString() + " input:"+ InputFile+" output:"+ IntermediatePath);

            if (ParsedCompilerOptions.ContainsKey("/Yc")) //Create PCH
            {
                //Console.WriteLine("create pch co:"+InputFile);
            ...
            {
                //Console.WriteLine("use pch co:" + InputFile);
                ...
                else if (InputFile.EndsWith(".cc")){
                    CompilerOutputExtension = ".cc.obj";
                }
            }
            else
            {
        ...
        private void AddLinkAction(List<Action> Actions, int ActionIndex, List<int> DependencyIndices)
        {
            ...
            Console.WriteLine("link object index:"+ ActionIndex+" options:" + string.Join(",", ParsedLinkerOptions.Keys) + " is mvc:" + IsMSVC().ToString() +" dep count:"+ DependencyIndices.Count + " response file:" + ResponseFilePath + " output:" + OutputFile?.ToString() + " arguments:" + Action.CommandArguments);
            ...
            else if (Action.CommandPath.FullName.Contains("link-filter.exe"))
            {
                //
                string Linker = "";
                string[] cas = Action.CommandArguments.Split('@');
                if (cas.Length > 1)
                {
                    Linker = cas[0].Trim();
                }
                ...
                if (DependencyIndices.Count == 0)
                {
                    ...
                    if (IsMSVC())
                    {
                        ...
                        {
                            //link-filter.exe -linker=<linker-file> -- <child command line>
                            // /ignore:4042 to turn off the linker warning about the output option being present twice (command-line + rsp file)
                            AddText(string.Format("\t.LinkerOptions = '/TLBOUT:\"%1\" /ignore:4042 /Out:\"%2\" @\"{0}\" ' \n", OtherCompilerOptions)); // The TLBOUT is a huge bodge to consume the %1.
                        }
                    }
                    else
                ...
                else
                {
                    ...
                    if (IsMSVC())
                    {
                        ...
                        {
                            AddText(string.Format("\t.LinkerOptions = ' {1} /TLBOUT:\"%1\" /Out:\"%2\" @\"{0}\" ' \n", ResponseFilePath, Linker)); // The TLBOUT is a huge bodge to consume the %1.
                        }
                    }
                    else
                        AddText(string.Format("\t.LinkerOptions = '{0} -o \"%2\" @\"%1\"' \n", OtherCompilerOptions)); // The MQ is a huge bodge to consume the %1.
            ...
            else if (Action.CommandArguments.Contains("link.exe") || Action.CommandPath.FullName.Contains("orbis-clang"))
            {
                if (DependencyIndices.Count > 0) //Insert a dummy node to make sure all of the dependencies are finished.
                                                 //If FASTBuild supports PreBuildDependencies on the Executable action we can remove this.
                {
        ...
        private bool CreateBffFile(List<Action> InActions, string BffFilePath, List<Action> LocalExecutorActions)
        {
            ...
            try
            {
                ...
                List<int> actionTargetList = new List<int>();
                ...
                for (int ActionIndex = 0; ActionIndex < Actions.Count; ActionIndex++)
                {
                    ...
                    //Console.WriteLine($"Action check index:{ActionIndex} FastBuildActionsCount:{actionTargetList.Count} type:{Action.ActionType}");
                    AddText(string.Format("// cmd:\"{0}\" arguments:{1}\n", Action.CommandPath, Action.CommandArguments));
                    switch (Action.ActionType)
                    {
                        case ActionType.Compile:
                            AddCompileAction(Action, ActionIndex, DependencyIndices);
                            actionTargetList.Add(ActionIndex);
                            break;
                        case ActionType.Link:
                            if (bDisableRemoteLink)
                            {
                ...
                AddText("Alias( 'all' ) \n{\n");
                AddText("\t.Targets = { \n");
                for (int ActionIndex = 0; ActionIndex < actionTargetList.Count; ActionIndex++)
                {
                    AddText(string.Format("\t\t'Action_{0}'{1}", actionTargetList[ActionIndex], ActionIndex < (actionTargetList.Count - 1) ? ",\n" : "\n\t}\n"));
                }
                AddText("}\n");
        ...
        private bool ExecuteBffFile(string BffFilePath)
        {
            ...
            string distArgument = bEnableDistribution ? "-dist -distverbose" : "";
            string cleanArg = bEnableClean ? "-clean" : "";
            ...
            //-clean 
            string FBCommandLine = string.Format("-monitor -summary {0} {1} {2} -ide -config \"{3}\"", distArgument, cacheArgument, cleanArg, BffFilePath);
		...
		private void GenerateDependencyFiles(List<Action> Actions, string ReportFilePath)
		{
			...
			{
				Log.TraceWarning("FASTBuild report.html:{0} not found, skipping dependency files generation", ReportFilePath);
				return;
			}
